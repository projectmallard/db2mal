...
<xsl:template mode="db.info.inline.mode" match="*">
  <xsl:message>
    <xsl:text>Unmatched info inline element: </xsl:text>
    <xsl:value-of select="local-name(.)"/>
  </xsl:message>
  <xsl:apply-templates mode="db.info.inline.mode"/>
</xsl:template>
...
<xsl:template mode="l10n.format.mode" match="msg:copyright.years">
  ...
  <xsl:for-each select="$node/year | $node/db:year">
    ...
    <xsl:apply-templates mode="db.info.inline.mode" select="."/>
  </xsl:for-each>
</xsl:template>
...
<xsl:template name="db.personname">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$node/db:orgname">
      <xsl:apply-templates mode="db.info.inline.mode" select="$node/db:orgname"/>
    </xsl:when>
    <xsl:when test="$node/self::collab or $node/self::db:collab">
      <xsl:apply-templates mode="db.info.inline.mode"
                           select="$node/collabname |
                                   $node/db:org | $node/db:orgname |
                                   $node/db:person/db:personname |
                                   $node/db:personname"/>
    </xsl:when>
    <xsl:when test="$node/self::corpauthor or $node/self::corpcredit">
      <xsl:apply-templates mode="db.info.inline.mode" select="$node"/>
    </xsl:when>
    <xsl:when test="$node/self::db:personname and not($node/db:surname)">
      <xsl:apply-templates mode="db.info.inline.mode" select="$node/node()"/>
    </xsl:when>
    <!-- family-given -->
    <xsl:when test="$node/@role = 'family-given'">
      <xsl:apply-templates mode="db.info.inline.mode"
                           select="($node/surname | $node/db:surname)[1]"/>
      <xsl:if test="$node/surname | $node/db:surname">
        <xsl:text> </xsl:text>
      </xsl:if>
      <xsl:apply-templates  mode="db.info.inline.mode"
                            select="($node/firstname | $node/db:firstname)[1]"/>
    </xsl:when>
    <!-- last-first -->
    ...
    <xsl:otherwise>
      <xsl:if test="$node/honorific or $node/db:honorific">
        <xsl:apply-templates  mode="db.info.inline.mode"
                              select="($node/honorific | $node/db:honorific)[1]"/>
      </xsl:if>
      <xsl:if test="$node/firstname or $node/db:firstname">
        ...
        <xsl:apply-templates  mode="db.info.inline.mode"
                              select="$node/firstname[1] | $node/db:firstname[1]"/>
      </xsl:if>
      <xsl:if test="$node/othername or $node/db:othername">
        ...
        <xsl:apply-templates  mode="db.info.inline.mode"
                              select="$node/othername[1] | $node/db:othername[1]"/>
      </xsl:if>
      <xsl:if test="$node/surname or $node/db:surname">
        ...
        <xsl:apply-templates  mode="db.info.inline.mode"
                              select="$node/surname[1] | $node/db:surname[1]"/>
      </xsl:if>
      <xsl:if test="$node/lineage or $node/db:lineage">
        <xsl:text>, </xsl:text>
        <xsl:apply-templates  mode="db.info.inline.mode"
                              select="$node/lineage[1] | $node/db:lineage[1]"/>
      </xsl:if>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
